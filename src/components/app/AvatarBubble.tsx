...
export function AvatarBubble({ user, size = 56, ringClass, label, onClick }: Props) {
  ...
  return (
    ...
    >
      <div
        className={`flex items-center justify-center overflow-hidden rounded-full border-2 border-background shadow-card transition-transform group-hover:scale-110 ${ringClass ?? ""}`}
        style={{
          width: size,
          height: size,
